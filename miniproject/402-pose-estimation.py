...
input_layer = compiled_model2.input(0)
output_layers = compiled_model2.outputs
...
def run_pose_estimation(source=0, flip=False, use_popup=False, skip_first_frames=0):
    ...
    try:
        ...
        while True:
            ...
            # Get results.
            results = compiled_model2([input_img])
            stop_time = time.time()
            ...
            if poses[-1][10][1] <= poses[-1][8][1] and poses[-1][9][1] <= poses[-1][7][1]:
                cv2.putText(frame, "Vingo", (100,100),cv2.FONT_HERSHEY_COMPLEX, f_width / 1000, (0, 0, 255), 1, cv2.LINE_AA)
            ...
            if use_popup:
                ...
                #     points = pose[:, :2].astype(np.int32)
                #     points_scores = pose[:, 2]
                #     predicted_positions = np.array([[345, 206], [362, 183], [322, 176], [395, 198], [300, 191],
                #                                       [429, 288], [255, 311], [558, 363], [204, 438], [558, 258]])
                #     for i, (p, v) in enumerate(zip(points, points_scores)):
                #         if v > 0.1:
                
                #         # 실제 좌표
                #             actual_positions = p.tolist()
                #                 # 허용 가능한 최대 오차
                #             max_error = 10
                #                 # 각 좌표 간의 거리 계산
                #             distances = np.sqrt(np.sum((predicted_positions - actual_positions)**2, axis=1))
                #                 # 각 좌표의 오차를 확인하고 필요에 따라 처리
                #             if all(distance <= max_error for distance in distances):
                #                     print("빙고 발견!")
                #             else:
                #                     print("빙고 아님")
                #  각 포즈 위치만으로는 찾기는 힘듬
